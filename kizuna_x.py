#!/usr/bin/env python3
##  KIZUNA X  v5.0  —  50 Tools: OSINT · Media · Fun · Utilities
##  Author: mohmmadsedeg30-design  |  License: MIT
##  Fixed: Prompt.ask() args, importlib.util, all tools working

import os, sys, json, socket, hashlib, logging, ipaddress, threading
import time, random, base64, urllib.parse, shutil, platform, ssl
from datetime import datetime
from concurrent.futures import ThreadPoolExecutor
from typing import Optional

# ── smart install: فقط المكتبات الخفيفة تلقائياً ─────────────
def _install(pkg):
    os.system(f"pip install {pkg} --break-system-packages -q 2>/dev/null || "
              f"pip install {pkg} -q 2>/dev/null")

LIGHT = ["requests","phonenumbers","dnspython","email-validator",
         "python-whois","python-dotenv","rich","pyfiglet","qrcode[pil]",
         "colorama","Pillow"]

for pkg in LIGHT:
    mod = pkg.split("[")[0].replace("-","_").replace("python_","")
    mod_map = {"dnspython":"dns","email_validator":"email_validator",
               "python_whois":"whois","python_dotenv":"dotenv",
               "qrcode":"qrcode","Pillow":"PIL"}
...
logging.basicConfig(
    filename="logs/kizuna.log", level=logging.INFO,
    format="%(asctime)s | %(levelname)-8s | %(message)s",
    datefmt="%Y-%m-%d %H:%M:%S")
console = Console()

# ── opencv: اختياري بسبب الحجم ───────────────────────────────
try:
    import cv2
    HAS_CV2 = True
except ImportError:
...
██║  ██╗██║███████╗╚██████╔╝██║ ╚████║██║  ██║
╚═╝  ╚═╝╚═╝╚══════╝ ╚═════╝ ╚═╝  ╚═══╝╚═╝  ╚═╝
      50 Tools · OSINT · Media · Fun · Utils  v5.0
"""
...
def ask(prompt, default=None, choices=None):
    """Prompt.ask wrapper — fixes version compatibility"""
    kwargs = {}
...
def http_get(url, params=None, timeout=10):
    try:
        return requests.get(url, params=params, timeout=timeout,
                            headers={"User-Agent": "KizunaX/5.0"})
    except Exception:
        return None
...
def save_json(name, data):
    ...
    p  = f"results/{name}_{ts}.json"
    with open(p, "w", encoding="utf-8") as f:
        json.dump(data, f, indent=4, default=str, ensure_ascii=False)
    console.print(f"[dim]💾 saved → {p}[/dim]")

def do_save(data, name):
    if ask("\n[dim]Save result?[/dim]", default="n", choices=["y","n"]) == "y":
        save_json(name, data)

ASCII_CHARS = "@%#*+=-:. "
def resize_img(img, w=100):
    ow, oh = img.size
    return img.resize((w, max(1, int(w*(oh/ow)/2.0))))
def p2a(v): return ASCII_CHARS[min(v * len(ASCII_CHARS) // 256, len(ASCII_CHARS)-1)]
...
def t01_phone():
    ...
    n = ask("[cyan]Phone number[/cyan] (e.g. +966501234567)")
    try:
        ...
        TYPES = {0:"FIXED",1:"MOBILE",2:"FIXED/MOBILE",3:"TOLL_FREE",6:"VOIP",7:"PERSONAL"}
        rows  = [
            ("E164",         e164),
            ("International",intl),
        ...
        if key:
            r = http_get("http://apilayer.net/api/validate",
                         params={"access_key":key,"number":e164})
            if r and r.ok:
                d = r.json()
                for k in ("carrier","line_type","location"):
                    if d.get(k): rows.append((k.title(), d[k]))
        console.print(make_table(f"PHONE: {n}", rows))
        do_save(dict(rows), "phone")
        log("PHONE", n)
...
def t02_email():
    ...
    email = ask("[green]Email address[/green]")
    try:
        ...
        for lb, qn in [("SPF",""),("DMARC","_dmarc."),("DKIM","default._domainkey.")]:
            try:
                ...
                rows.append((lb, "Not found"))
        try:
            r = http_get("https://disposable.github.io/disposable-email-domains/domains.json",timeout=5)
            if r: rows.append(("Disposable","⚠️ YES" if d in set(r.json()) else "✅ NO"))
        except Exception: pass
        console.print(make_table(f"EMAIL: {email}", rows, style="green"))
        do_save(dict(rows), "email")
        log("EMAIL", email)
...
def t03_ip():
    ...
    raw = ask("[magenta]IP or hostname[/magenta]")
    try:
        ipaddress.ip_address(raw); ip = raw
    except ValueError:
    ...
    console.print(make_table(f"IP: {ip}", rows, style="magenta"))
    do_save(dict(rows), "ip")
    log("IP", ip); pause()

def t04_domain():
    section("🌐  DOMAIN OSINT", "yellow")
    t = ask("[yellow]Domain[/yellow]"); rows = []
    try:
        info = whois.whois(t)
    ...
    for rt in ["A","AAAA","MX","NS","TXT","SOA","CAA"]:
        try:
            for a in dns.resolver.resolve(t, rt, lifetime=4):
                rows.append((f"DNS {rt}", str(a).rstrip(".")[:80]))
        except Exception: pass
    try:
        ...
        if r:
            ...
            for i,s in enumerate(sorted(subs)[:15]):
                rows.append((f"Sub {i+1}", s))
    except Exception: pass
    console.print(make_table(f"DOMAIN: {t}", rows, style="yellow"))
    do_save(rows, "domain"); log("DOMAIN", t); pause()

def t05_dns():
    section("📡  DNS DEEP SCAN", "bright_cyan")
    t = ask("[bright_cyan]Domain[/bright_cyan]"); rows = []
    for rt in ["A","AAAA","CNAME","MX","NS","TXT","SOA","CAA","DNSKEY","SRV"]:
        try:
            for a in dns.resolver.resolve(t, rt, lifetime=5):
                rows.append((rt, str(a).rstrip(".")))
        except Exception: pass
    try:
        for ns in dns.resolver.resolve(t, "NS", lifetime=4):
            ...
            except Exception: rows.append(("AXFR","✅ Refused"))
    except Exception: pass
    console.print(make_table(f"DNS: {t}", rows, headers=("TYPE","VALUE"), style="bright_cyan"))
    do_save(rows,"dns"); log("DNS",t); pause()

WORDLIST = ["www","mail","api","dev","staging","admin","cdn","app","portal",
            "vpn","ftp","smtp","ns1","ns2","shop","blog","support","git",
            "dashboard","test","auth","login","m","media","static"]
def t06_subdomains():
    section("🔭  SUBDOMAIN ENUM", "bright_green")
    domain = ask("[bright_green]Domain[/bright_green]"); found = {}
            ...
            for e in r.json():
                for n in e.get("name_value","").split("\n"):
                    ...
                    if n.endswith(f".{domain}") and "*" not in n:
                        found.setdefault(n,[]).append("crt.sh")
    except Exception: pass
    console.print("[yellow]► DNS bruteforce…[/yellow]")
    lock = threading.Lock()
    def chk(sub):
        fqdn = f"{sub}.{domain}"
        try:
    ...
    console.print(make_table(f"SUBDOMAINS: {domain} ({len(rows)})", rows,
                             headers=("SUBDOMAIN","SOURCE"), style="bright_green"))
    do_save(found,"subdomains"); log("SUBDOMAIN",domain); pause()

def t07_ssl():
    section("🔒  SSL CERTIFICATE", "bright_white")
    host = ask("[white]Domain[/white]")
    port = int(ask("Port", default="443"))
    try:
        ...
        console.print(make_table(f"SSL: {host}", rows, style="bright_white"))
        do_save(dict(rows),"ssl")
...
def t09_url_expand():
    ...
    url = ask("[cyan]Short URL[/cyan]")
    try:
        r = requests.get(url, allow_redirects=True, timeout=10)
        rows = [("Original",url),("Final",r.url),
                ("Status",str(r.status_code)),("Hops",str(len(r.history)))]
...
def t10_http_headers():
    ...
    url = ask("[yellow]URL[/yellow]")
    if not url.startswith("http"): url = "https://" + url
    try:
        r = requests.get(url, timeout=10, headers={"User-Agent":"KizunaX/5.0"})
...
def t12_img_ascii_color():
    ...
    path = ask("Image path").strip()
    if not os.path.isfile(path): console.print("[red]Not found.[/red]"); pause(); return
    w = int(ask("Width chars", default="80"))
    try:
...
def t13_img_blocks():
    ...
    path = ask("Image path").strip()
    if not os.path.isfile(path): console.print("[red]Not found.[/red]"); pause(); return
    w = int(ask("Width blocks", default="60"))
    try:
...
def t14_video_ascii_color():
    ...
    if not HAS_CV2:
        console.print("[yellow]Installing opencv (one time, ~100MB)…[/yellow]")
        _install("opencv-python-headless")
        console.print("[green]Done! Restart and try again.[/green]"); pause(); return
    path = ask("Video path").strip()
    if not os.path.isfile(path): console.print("[red]Not found.[/red]"); pause(); return
    w   = int(ask("Width chars", default="80"))
    fps = int(ask("Max FPS", default="10"))
    cap = cv2.VideoCapture(path)
    if not cap.isOpened(): console.print("[red]Cannot open video.[/red]"); pause(); return
    ...
    try:
        while True:
            ...
            img = resize_img(Image.fromarray(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)), w)
...
def t15_video_ascii_bw():
    ...
    if not HAS_CV2:
        console.print("[yellow]opencv not installed. Run tool 14 first.[/yellow]"); pause(); return
    path = ask("Video path").strip()
    if not os.path.isfile(path): console.print("[red]Not found.[/red]"); pause(); return
    w   = int(ask("Width chars", default="100"))
    fps = int(ask("Max FPS", default="10"))
    cap = cv2.VideoCapture(path)
    if not cap.isOpened(): console.print("[red]Cannot open video.[/red]"); pause(); return
    vfps = cap.get(cv2.CAP_PROP_FPS) or 24; skip = max(1, int(vfps/fps)); fn = 0
    try:
        while True:
            ...
            fn += 1
            if fn % skip != 0: continue
            img = resize_img(Image.fromarray(cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)).convert("L"), w)
            pixels = list(img.getdata()); iw = img.width; art = ""
            for i,p in enumerate(pixels):
                art += p2a(p)
                if (i+1) % iw == 0: art += "\n"
...
def t16_img_info():
    section("ℹ️  IMAGE INFO + EXIF", "white")
    path = ask("Image path").strip()
    if not os.path.isfile(path): console.print("[red]Not found.[/red]"); pause(); return
    try:
        img  = Image.open(path)
        rows = [("File", os.path.basename(path)),("Format",img.format or "—"),
                ("Mode",img.mode),("Size",f"{img.width}×{img.height}px"),
                ("File Size",f"{os.path.getsize(path)//1024} KB")]
        try:
...
def t17_img_resize():
    section("📐  IMAGE RESIZE", "cyan")
    path = ask("Image path").strip()
    if not os.path.isfile(path): console.print("[red]Not found.[/red]"); pause(); return
    w = int(ask("Width px")); h = int(ask("Height px"))
    try:
...
def t19_img_negative():
    section("🔄  IMAGE → NEGATIVE", "bright_white")
    path = ask("Image path").strip()
    if not os.path.isfile(path): console.print("[red]Not found.[/red]"); pause(); return
    try:
        out = f"output/negative_{os.path.basename(path)}"
        ImageOps.invert(Image.open(path).convert("RGB")).save(out)
        console.print(f"[green]✅ Saved → {out}[/green]")
    except Exception as e: console.print(f"[red]{e}[/red]")
    pause()
...
def t20_qr():
    ...
    data = ask("[green]Text / URL[/green]")
    name = ask("Filename", default="qr")
    try:
        qr = qrcode.QRCode(version=1, box_size=10, border=4)
        qr.add_data(data); qr.make(fit=True)
        out = f"output/{name}.png"
        qr.make_image(fill_color="black", back_color="white").save(out)
        console.print(f"[green]✅ QR saved → {out}[/green]")
    except Exception as e: console.print(f"[red]{e}[/red]")
    pause()
...
def t21_ascii_text():
    ...
    text  = ask("[yellow]Text[/yellow]")
    font  = ask("Font (slant/banner3/doom/big/block)", default="slant")
    try:
        art   = pyfiglet.figlet_format(text, font=font)
        color = random.choice(["cyan","magenta","yellow","green","bright_blue","bright_red"])
        console.print(f"[bold {color}]{art}[/bold {color}]")
        if ask("Save?", default="n", choices=["y","n"]) == "y":
...
def t22_rainbow():
    ...
    text = ask("[magenta]Text[/magenta]")
    big  = ask("Big ASCII art?", default="y", choices=["y","n"])
...
def t23_matrix():
    section("💚  MATRIX RAIN", "green")
    dur  = int(ask("Duration (seconds)", default="10"))
    ...
    try:
        while time.time() < end:
            ...
            for i,drop in enumerate(drops):
                if drop > 0 and random.random() > 0.5:
...
def t24_hacker_typer():
    ...
    console.print("[dim]Press any key to type code. Ctrl+C to stop.[/dim]\n")
...
def t28_countdown():
    section("⏱️  COUNTDOWN TIMER", "yellow")
    sec = int(ask("[yellow]Seconds[/yellow]"))
    try:
        for r in range(sec, 0, -1):
            os.system("clear")
            console.print(f"[bold yellow]{pyfiglet.figlet_format(str(r), font='big')}[/bold yellow]")
            time.sleep(1)
        os.system("clear")
        console.print(f"[bold green]{pyfiglet.figlet_format('DONE!', font='big')}[/bold green]")
        print("\a")
    except KeyboardInterrupt: pass
    pause()
...
def t29_passgen():
    import string
    section("🔐  PASSWORD GENERATOR", "bright_green")
    length = int(ask("Length", default="16"))
    count  = int(ask("Count",  default="5"))
    sym    = ask("Include symbols?", default="y", choices=["y","n"])
    chars  = string.ascii_letters + string.digits + ("!@#$%^&*()-_=+" if sym=="y" else "")
    rows   = [(str(i+1), "".join(random.choices(chars, k=length))) for i in range(count)]
    console.print(make_table("PASSWORDS", rows, headers=("#","PASSWORD"), style="bright_green"))
    pause()
...
def t30_joke():
    ...
    console.print(Panel(f"[bold yellow]{q}[/bold yellow]", title="❓ Question", border_style="yellow"))
    ask("[dim]Press ENTER for answer[/dim]", default="")
    console.print(Panel(f"[bold green]{a}[/bold green]", title="💡 Answer", border_style="green"))
    pause()
...
def t31_quote():
    ...
    try:
        ...
        if r and r.ok:
            d = r.json()
            console.print(Panel(
                f'[italic cyan]"{d["content"]}"[/italic cyan]\n\n[dim]— {d["author"]}[/dim]',
                title="Quote", border_style="cyan", padding=(1,4)))
...
def t32_base64():
    section("🔤  BASE64 TOOL", "blue")
    console.print("[1] Encode  [2] Decode")
    c    = ask("Select", choices=["1","2"])
    text = ask("Input")
    try:
...
def t33_encoder():
    ...
    c    = ask("Select", choices=["1","2","3","4"])
    text = ask("Input")
    if c == "1":
        sh = int(ask("Shift", default="13"))
        r  = "".join(
            chr((ord(ch)-ord('A')+sh)%26+ord('A')) if ch.isupper() else
            chr((ord(ch)-ord('a')+sh)%26+ord('a')) if ch.islower() else ch
            for ch in text)
        console.print(f"\n[green]{r}[/green]")
    elif c == "2":
...
def t34_units():
    section("📏  UNIT CONVERTER", "cyan")
    console.print("[1] Temperature  [2] Length  [3] Weight  [4] Data Size")
    c = ask("Select", choices=["1","2","3","4"])
    v = float(ask("Value"))
    if c == "1":
        console.print(make_table("TEMPERATURE",[
            ("°C → °F", f"{v*9/5+32:.2f}"),("°C → K",  f"{v+273.15:.2f}"),
            ("°F → °C", f"{(v-32)*5/9:.2f}"),("K → °C",  f"{v-273.15:.2f}")]))
    elif c == "2":
        console.print(make_table("LENGTH",[
            ("km → miles",f"{v*0.621371:.4f}"),("miles → km",f"{v*1.60934:.4f}"),
            ("m → ft",    f"{v*3.28084:.4f}"), ("ft → m",    f"{v*0.3048:.4f}"),
            ("inch → cm", f"{v*2.54:.4f}"),    ("cm → inch", f"{v/2.54:.4f}")]))
    elif c == "3":
        console.print(make_table("WEIGHT",[
            ("kg → lbs",f"{v*2.20462:.4f}"),("lbs → kg",f"{v*0.453592:.4f}"),
            ("g → oz",  f"{v*0.035274:.4f}"),("oz → g",  f"{v*28.3495:.4f}")]))
    elif c == "4":
        console.print(make_table("DATA SIZE",[
            ("Bytes → KB", f"{v/1024:.4f}"),    ("Bytes → MB", f"{v/1048576:.4f}"),
            ("Bytes → GB", f"{v/1073741824:.4f}"),("KB → MB",  f"{v/1024:.4f}"),
            ("MB → GB",    f"{v/1024:.4f}"),    ("GB → TB",    f"{v/1024:.4f}")]))
    pause()

def t35_base_conv():
    section("🔢  NUMBER BASE CONVERTER", "blue")
    inp = ask("[blue]Input number[/blue]")
    frm = int(ask("From base (2/8/10/16)", default="10"))
    try:
        n = int(inp, frm)
        console.print(make_table("BASE CONVERSION",[
            ("Binary (2)",   bin(n)[2:]),("Octal (8)",    oct(n)[2:]),
            ("Decimal (10)", str(n)),    ("Hex (16)",     hex(n)[2:].upper())],
            style="blue"))
    except Exception as e: console.print(f"[red]{e}[/red]")
    pause()

def t36_text_stats():
    section("📊  TEXT STATISTICS", "green")
    console.print("[dim]Paste text then press Enter twice:[/dim]")
    lines = []
    while True:
    ...
    console.print(make_table("TEXT STATS",[
        ...
        ("Unique words",    str(len(set(w.lower() for w in words)))),
    ], style="green")); pause()

def t37_url_enc():
    section("🔗  URL ENCODE / DECODE", "cyan")
    console.print("[1] Encode  [2] Decode")
    c    = ask("Select", choices=["1","2"])
    text = ask("Input")
    if c == "1": console.print(f"\n[green]{urllib.parse.quote(text)}[/green]")
    else:        console.print(f"\n[green]{urllib.parse.unquote(text)}[/green]")
    pause()

def t38_ping():
    section("📶  PING", "cyan")
    host  = ask("[cyan]Host[/cyan]")
    count = ask("Count", default="4")
    os.system(f"ping -c {count} {host}" if os.name!="nt" else f"ping -n {count} {host}")
    pause()

def t39_traceroute():
    section("🗺️  TRACEROUTE", "magenta")
    host = ask("[magenta]Host[/magenta]")
    os.system(f"traceroute {host}" if os.name!="nt" else f"tracert {host}")
    pause()

def t40_dns_quick():
    section("📡  DNS QUICK LOOKUP", "bright_cyan")
    domain = ask("[cyan]Domain[/cyan]")
    rtype  = ask("Record type", default="A").upper()
    try:
...
def t41_whois():
    section("🌐  WHOIS QUICK", "yellow")
    t = ask("[yellow]Domain / IP[/yellow]")
    try:
        info = whois.whois(t)
        rows = [(k.replace("_"," ").title(), str(v)[:80]) for k,v in info.items() if v][:20]
        console.print(make_table(f"WHOIS: {t}", rows, style="yellow"))
    except Exception as e: console.print(f"[red]{e}[/red]")
    pause()

def t42_binary():
    section("💾  TEXT ↔ BINARY", "blue")
    console.print("[1] Text → Binary  [2] Binary → Text")
    c    = ask("Select", choices=["1","2"])
    text = ask("Input")
    if c == "1":
...
def t43_hexdump():
    section("🔍  HEX DUMP", "bright_white")
    console.print("[1] Text → Hex  [2] File Hex Dump")
    c = ask("Select", choices=["1","2"])
    if c == "1":
        text = ask("Text")
        console.print(f"\n[green]{' '.join(format(ord(ch),'02X') for ch in text)}[/green]")
    else:
        p = ask("File path").strip()
        if not os.path.isfile(p): console.print("[red]Not found.[/red]"); pause(); return
        with open(p,"rb") as f: data = f.read(512)
        for i in range(0, len(data), 16):
            chunk = data[i:i+16]
            console.print(f"[dim]{i:04X}[/dim]  "
                          f"{' '.join(f'{b:02X}' for b in chunk):<48}  "
                          f"[cyan]{''.join(chr(b) if 32<=b<127 else '.' for b in chunk)}[/cyan]")
    pause()

def t44_colorconv():
    section("🎨  COLOR CODE CONVERTER", "bright_magenta")
    console.print("[1] HEX → RGB  [2] RGB → HEX")
    c = ask("Select", choices=["1","2"])
    if c == "1":
        h = ask("HEX color (#rrggbb)").strip("#")
        try:
        ...
        r = int(ask("R (0-255)")); g = int(ask("G (0-255)")); b = int(ask("B (0-255)"))
...
def t45_random_data():
    import uuid, string
    section("🎲  RANDOM DATA GENERATOR", "bright_green")
    console.print(make_table("RANDOM DATA",[
        ...
        ("PIN (6)",     "".join(random.choices(string.digits,k=6))),
        ("Coin flip",   random.choice(["HEADS 🪙","TAILS 🪙"])),
        ("Dice d6",     str(random.randint(1,6))),
        ("Dice d20",    str(random.randint(1,20))),
        ("Dice d100",   str(random.randint(1,100))),
...
def t47_hash():
    ...
    console.print("[1] Hash text  [2] Hash file  [3] Compare  [4] Identify type")
    c = ask("Select", choices=["1","2","3","4"])
    ALGOS = {"MD5":hashlib.md5,"SHA1":hashlib.sha1,"SHA256":hashlib.sha256,
             "SHA512":hashlib.sha512,"SHA3-256":hashlib.sha3_256,"BLAKE2b":hashlib.blake2b}
    if c == "1":
        text = ask("Text")
        rows = [(a, fn(text.encode()).hexdigest()) for a,fn in ALGOS.items()]
        console.print(make_table("HASHES", rows, headers=("ALGORITHM","HASH"), style="blue"))
    elif c == "2":
        p = ask("File path").strip()
        if not os.path.isfile(p): console.print("[red]Not found.[/red]"); pause(); return
        ...
        console.print(make_table(f"FILE: {os.path.basename(p)}", rows,
                                 headers=("ALGORITHM","HASH"), style="blue"))
    elif c == "3":
        h1 = ask("Hash 1").strip().lower()
        h2 = ask("Hash 2").strip().lower()
        console.print("[green]✅ MATCH[/green]" if h1==h2 else "[red]❌ MISMATCH[/red]")
    elif c == "4":
        h = ask("Hash").strip()
        LENS = {32:"MD5",40:"SHA-1",56:"SHA-224",64:"SHA-256",96:"SHA-384",128:"SHA-512"}
        console.print(f"\nLength {len(h)} → likely [bold]{LENS.get(len(h),'Unknown')}[/bold]")
    pause()
...
def t48_sysinfo():
    ...
    rows = [
        ...
        ("Python",      sys.version.split()[0]),("CPU cores", str(os.cpu_count())),
    ]
    try: rows.append(("Local IP", socket.gethostbyname(socket.gethostname())))
    except Exception: pass
    try:
    ...
    try:
        ...
        rows.append(("Disk", f"{du.total//1073741824}GB total / {du.free//1073741824}GB free"))
    except Exception: pass
    console.print(make_table("SYSTEM INFO", rows)); pause()

def t49_speedtest():
    section("⚡  INTERNET SPEED TEST", "yellow")
    console.print("[yellow]Testing… (20–30 seconds)[/yellow]")
    try:
        import speedtest
        st = speedtest.Speedtest()
        with console.status("Finding best server…"): st.get_best_server()
        with console.status("Testing download…"):    dl = st.download() / 1e6
        with console.status("Testing upload…"):      ul = st.upload()   / 1e6
        rows = [
            ("Download", f"{dl:.2f} Mbps"),("Upload", f"{ul:.2f} Mbps"),
            ("Ping",     f"{st.results.ping:.1f} ms"),
            ("ISP",      st.results.client.get("isp","?")),
        ]
        console.print(make_table("SPEED TEST", rows, style="yellow"))
    except ImportError:
        console.print("[yellow]Installing speedtest-cli…[/yellow]")
        _install("speedtest-cli")
        console.print("[green]Done! Run again.[/green]")
    except Exception as e:
        console.print(f"[red]{e}[/red]")
    pause()

def t50_about():
    section("ℹ️  ABOUT KIZUNA X", "bright_cyan")
    ...
    console.print(make_table("ABOUT",[
        ...
        ("Fixed in v5","Prompt.ask args · importlib.util · all tools"),
...
CATS = [
    ("🔍 OSINT", [
        ("1","📱 Phone OSINT",t01_phone),("2","📧 Email OSINT",t02_email),
    ...
    ("🖼️ MEDIA", [
        ("11","🖼️ ASCII B&W",t11_img_ascii_bw),("12","🎨 ASCII Color",t12_img_ascii_color),
        ("13","🟦 Block Art",t13_img_blocks),("14","🎬 Video Color",t14_video_ascii_color),
        ("15","🎬 Video B&W",t15_video_ascii_bw),("16","ℹ️ Image Info",t16_img_info),
        ("17","📐 Resize",t17_img_resize),("18","⬛ Grayscale",t18_img_gray),
        ("19","🔄 Negative",t19_img_negative),("20","📲 QR Code",t20_qr),
    ]),
    ("🎨 FUN", [
        ("21","✍️ ASCII Text",t21_ascii_text),("22","🌈 Rainbow",t22_rainbow),
    ...
    ("🛠️ UTILITIES", [
        ("31","💬 Random Quote",t31_quote),("32","🔤 Base64",t32_base64),
        ("33","🔡 Text Encoder",t33_encoder),("34","📏 Unit Converter",t34_units),
        ("35","🔢 Base Converter",t35_base_conv),("36","📊 Text Stats",t36_text_stats),
        ("37","🔗 URL Encode",t37_url_enc),("38","📶 Ping",t38_ping),
        ("39","🗺️ Traceroute",t39_traceroute),("40","📡 DNS Quick",t40_dns_quick),
    ]),
    ("🔬 MORE", [
        ("41","🌐 Whois",t41_whois),("42","💾 Text↔Binary",t42_binary),
...
def print_menu():
    ...
    console.print(f"  [bold]KIZUNA X[/bold]  [dim]v5.0 · 50 Tools · {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}[/dim]\n")
...
def main():
    console.print(Panel(f"[bold cyan]{BANNER}[/bold cyan]",
                        title="[bold]KIZUNA X[/bold]",
                        subtitle="[dim]v5.0 · 50 Tools · OSINT · Media · Fun · Utilities[/dim]",
                        border_style="cyan"))
    while True:
        print_menu()
        choice = ask("[bold]Tool number[/bold]")
        if choice == "0":
            console.print("\n[red]Goodbye.[/red]\n"); sys.exit(0)
        fn = ALL.get(choice)
        if fn: fn()
